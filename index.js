...
export const filterOceania = (countries) => {
  return countries.filter((countries) => countries.continent === 'Oceania');
};
...
export const filterAu = (countries) => {
  return countries.filter((countries) => countries.name.includes('au'));
};
...
export const getSneaks = (brands) => {
  const sneakerBrands = brands.reduce((acc, brand) => [...acc, ...brand.shoes], []);
  return sneakerBrands;
};
